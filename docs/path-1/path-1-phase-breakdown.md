...
| M2-03 Authoritative Event-Product Discovery | REUSE / EXTEND — **COMPLETE (PASS)** | ProductMapping, TicketType, native-v3 evidence patterns | `TickeraCatalogSync.queue_event_product_discovery/2` + `project_event_parent_products/3`; event-owned scope; parent-product projection | NO | NO | Bounded event-scoped discovery; no Apply |
