...
## Successor certificate handoff

After independent evidence review, copy only the allowlisted certificate fields from `evidence/EVIDENCE_TEMPLATE.md` into the closeout record. VS-26E.1 remains blocked unless the active deployed SHA, feed schema, exact run/hash, Apply/no-go, reconciled catalog counts, known exceptions, and independent verdict are all present.
