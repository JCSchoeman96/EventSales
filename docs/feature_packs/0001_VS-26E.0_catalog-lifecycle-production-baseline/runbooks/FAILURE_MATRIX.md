...
|---|---|---:|---|---|
| Current-main Railway status failing | GitHub deployment context failure | No blind redeploy | identify actual active SHA and diagnose read-only | Blocked |
| Finding payload exceeds operational review capacity | durable run/finding rows may still exist | No Apply | preserve evidence; open corrective hardening slice | Blocked |
| Full feed approaches row/page cap or causes DB pressure | bounded source run with performance symptoms | No limit increase | stop; profile/query-plan in separate approval | Blocked |
| Statistical latency evidence unavailable | one observed certification sample | N/A | record observed duration; do not claim p50/p95 | Conditional |
