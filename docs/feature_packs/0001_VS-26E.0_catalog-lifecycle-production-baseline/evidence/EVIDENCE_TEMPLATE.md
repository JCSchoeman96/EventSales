...
- GitHub main SHA:
- GitHub current-main deployment status:
...
- paging limits:
- calculated maximum aggregate rows:
- observed page count:
- observed row count:
- observed total duration:
- per-page latency evidence available: yes/no
- p50/p95 claimed: no (single certification run)
- finding message/metadata database-size bound: none
- maximum observed finding message bytes:
- maximum observed finding metadata encoded bytes:
...
- Summary counts:
- Finding counts by severity/code:
- Apply/no-go and timestamp:
- Reconciled catalog counts:
